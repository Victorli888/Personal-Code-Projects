...
def pop_check(A, B, C, D):   # Unused for now
    popped_89 = A.pop(5)
    if popped_89 != "89 Percent":
        Results.write(f" Pool Percentage {files89[i]} Fails to say 89%\n")
    popped_90 = A.pop(5)
    if popped_90 != "90 Percent":
        Results.write(f" Pool Percentage {files89[i]} Fails to say 90%\n")
    popped_91 = A.pop(5)
    if popped_91 != "91 Percent":
        Results.write(f" Pool Percentage {files89[i]} Fails to say 91%\n")
    popped_92 = A.pop(5)
    if popped_92 != "92 Percent":
        Results.write(f" Pool Percentage {files89[i]} Fails to say 92%\n")
    return A, B, C, D
...
# Create Text File
Results = open("xldf_compare_results.txt", "w+")
...
# Parse data from first XLDF starting with bet pool 1
